...
const revenue = sales.reduce((table,data)=>{
    if(table[data.category]){
        const {price,quantity} = data;
        const revenue = price * quantity;
        table[data.category].totalReveneu += revenue;
        table[data.category].itemCount += quantity;
    }
    else{
        table[data.category] = {totalReveneu:data.price*data.quantity, itemCount : data.quantity}
    }
    return table
},{})
